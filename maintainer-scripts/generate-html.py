...
print(
    color(f"reading raw post data from {RAW} ...", fg='yellow', style='bold'))
contexts = {}
feeds = {}
...
for k in feeds:
    ...
    context['CATEGORY'] = engine().plural(k.title())
...
for k in contexts:
    ...
    timestamp = datetime.strptime(
        contexts[k]['PUBLISH_DATE'], unprintStrptimeFmt).timestamp()
    all_posts.append((timestamp, contexts[k]))

all_posts = sorted(all_posts, key=lambda x: x[0], reverse=True)
# print(all_posts)
